<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		if ($module=='proforma' AND $act=='input'){
			...
			$desc_revenue=$_POST['desc_rev'];
			$revenue=$_POST['rate_rev'];
			$qty_revenue=$_POST['qty_rev'];

			$type_est_cost=$_POST['type_est_cost'];
			$desc_est_cost=$_POST['desc_est_cost'];
			$est_cost=$_POST['rate_cost'];
			$qty_est_cost=$_POST['qty_cost'];
			...
			for($x=0; $x < count($type_revenue); $x++) {
				$query="INSERT INTO pf_revenue (id_pf, type_revenue, desc_revenue, revenue, qty_revenue) VALUES ('$last_id','$type_revenue[$x]','$desc_revenue[$x]','$revenue[$x]','$qty_revenue[$x]')";
				$sql= mysql_query ($query) or die (mysql_error());
			}
		...
		elseif ($module=='proforma' AND $act=='tambah_revenue'){
			...
			$query="INSERT INTO pf_revenue (id_pf, type_revenue, desc_revenue, revenue, qty_revenue) VALUES ('$id_pf','$type_revenue','$desc_revenue','$revenue','$qty_revenue')";
			$sql= mysql_query ($query) or die (mysql_error());
		...
		//tambah qty
		else if ($module=='proforma' AND $act=='tambah_qty'){
			// Data pf_Party
			...
			$query="INSERT INTO pf_qty (id_pf, qty, type1, type2) VALUES ('$_GET[id]','$party_qty','$party_type1$party_pf1_desc','$party_type2$party_pf2_desc')";
			$sql= mysql_query ($query) or die (mysql_error());
			header("location:../../oklogin.php?module=$module&act=show&id=$_GET[id]");
		}
		//tambah qty
		else if ($module=='proforma' AND $act=='tambah_pudel'){
			// Data pf_pudel
			...
			$query="INSERT INTO pf_pudel (id_pf, pudel_date, qty, type1, type2, pudel_from, pudel_to) VALUES ('$_GET[id]','$pudel_date0','$pudel_party_qty','$pudel_party_pf1$pudel_party_pf1_desc','$pudel_party_pf2$pudel_party_pf2_desc','$pudel_route_from_pf','$pudel_route_to_pf')";
			$sql= mysql_query ($query) or die (mysql_error());
			header("location:../../oklogin.php?module=$module&act=show&id=$_GET[id]");
		}
		// Update ru
		...
		// Update Qty 
		elseif ($module=='proforma' AND $act=='update_qty'){
		...
		// Update Pudel
		elseif ($module=='proforma' AND $act=='update_pudel'){
		...
		// Hapus data
		elseif ($module=='proforma' AND $act=='delete_qty'){
			mysql_query("DELETE FROM pf_qty WHERE  id_pf_qty = " .$_GET['id']);
			header("location:../../oklogin.php?module=$module&act=show&id=$_GET[id_pf]");
		}
		elseif ($module=='proforma' AND $act=='delete_pudel'){
