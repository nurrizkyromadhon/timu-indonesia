			...
			for($x=0; $x < count($type_revenue); $x++) {
				...
				if ($save_rate == 'y') {
					...
					if (mysql_num_rows($query_rate) == 0) {
						$queryRate = "INSERT INTO ct_rev_rate (cust_name, shipment, type_rev, desc_rev, rate, ppn, pph) VALUES ('$cust_name','$shipment', '$type_revenue[$x]', '$desc1_revenue[$x]', '$rate_rev[$x]', '$ppn_rev[$x]', '$pph_rev[$x]')";
						$sqlRate = mysql_query ($queryRate) or die (mysql_error());
					}
				}

				$query="INSERT INTO pf_revenue (id_pf, type_revenue, desc_revenue, revenue, ppn_revenue, pph_revenue) VALUES ('$last_id','$type_revenue[$x]', '$desc1_revenue[$x]$desc2_revenue[$x]','$rate_rev[$x]','$ppn_rev[$x]','$pph_rev[$x]')";
				$sql= mysql_query ($query) or die (mysql_error());
				$rev_id = mysql_insert_id();
			}

			for($x=0; $x < count($type_est_cost); $x++) {
				$query="INSERT INTO pf_est_cost (id_pf, type_est_cost, desc_est_cost, est_cost, ppn_est_cost, pph_est_cost) VALUES ('$last_id','$type_est_cost[$x]', '$desc1_est_cost[$x]$desc2_est_cost[$x]','$rate_cost[$x]','$ppn_cost[$x]','$pph_cost[$x]')";
				$sql= mysql_query ($query) or die (mysql_error());
