<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		// Input user
		if ($module=='bukubesarjurnal' AND $act=='savejurnal'){
			if (isset($_POST['id_perkiraan'])) {
			...
			for($x=0; $x < count($id_perkiraan); $x++) {
					...
					$query0=mysql_query("SELECT * FROM jurnal as j join perkiraan as p on j.id_perkiraan=p.id_perkiraan WHERE id_jurnal ='$id_jurnal[$x]'");
					while ($hasil=mysql_fetch_array($query0)){
						$id_perkiraan2=$hasil['id_perkiraan'];
						$tgl=$hasil['tgl'];
					...
					if ($id_perkiraan[$x]!=''){
						...
						<?=$ket?><br>
						<?=$nominal?><br>
						<?=count($id_perkiraan)?><br><br>
						<?php
						
						$update=mysql_query("update jurnal set id_perkiraan2=$id_perkiraan[$x] where id_jurnal=$id_jurnal[$x]");
						
						$query = "INSERT INTO jurnal (user, tgl, no_jurnal, bgcek, id_perkiraan, id_perkiraan2, ket, nominal, dk)VALUES('$user','$tgl','$no_jurnal','$no_ref','$id_perkiraan[$x]','$id_perkiraan2','$ket','$nominal', '$dk')";
					...
					header('location:../../oklogin.php?module='.$module.'&kode_perkiraan='.$kode_perkiraan);
				}
				
		// Save excel
		elseif ($module=='akunting' AND $act=='save_excel'){	
		    ...
		    ?>
		                            <table border="1" class="table table-bordered table-striped">
												...
												<tbody>
													<?php
														$no=1;
														$query="SELECT *, sum(nominal) as jml FROM jurnal group by no_ref ORDER BY tgl desc";
