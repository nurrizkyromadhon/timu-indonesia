<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		// Input user
		if ($module=='keu' AND $act=='input'){
			if (isset($_POST['tanggal'])) {
				$user=$_SESSION['username'];
				$tgl_keu=$_POST['tanggal'];
				$no_ref=$_POST['no_ref'];
			...
			if (empty($no_ref)){
					...
					for($x=0; $x < count($nm_perkiraan); $x++) {
						...
						$query = "INSERT INTO keuangan (user, tgl, no_ref, id_perkiraan, ket, nominal, dk)VALUES('$user','$tgl_keu','$no_ref','$id_perkiraan','$ket[$x]','$nominal[$x]', '$dk[$x]')";
