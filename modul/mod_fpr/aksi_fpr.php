<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		if ($module=='batal' AND $act=='batal'){
			...
			//2. mencari baris pada profil_buyer untuk di ketahui id buyer  
			$query1=(mysql_query("select * from profil_buyer where nm_buyer = '$nama_user'"));
			$hasil1=mysql_fetch_array($query1);
			$id_buyer=$hasil1['id_buyer'];
			
			//3. insert / input pada tabel batal
			mysql_query("insert into batal (tgl_batal,no_batal,id_nasabah,ket) value ('$_POST[tgl]','$_POST[no_batal]','$id_buyer','$_POST[ket_batal]') ");
			...
			mysql_query("update nasabah set id_indikasi='1', id_harga='', tgl_beli='',nama_user='', harga_jual='', kpr='', marketing='', ket='', fpr='' where  id_nasabah='$id_nasabah'");

			// 7. Merubah keteranga di table harga untuk terjual untuk filter harga yg blom terjual		
			mysql_query("update harga set status='Terjual' where id_harga='$id_harga'");
			
			//4. mengambil id nasabah untuk inputan pada hut_user 
			//$query=(mysql_query("select * from nasabah where id_cord='$_POST[id_coord]'"));
			//$hasil=mysql_fetch_array($query) or die(mysql_error());
			//$id_nasabah=$hasil['id_nasabah'];
			
			//5. inputan pertama / baru pada hut_user 
