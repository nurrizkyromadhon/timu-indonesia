	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		if ($module=='invoice' AND $act=='tambah_invoice'){
			...
			for($x=0; $x < count($id_pf_revenue); $x++) {	
				...
				$query="INSERT INTO pf_invoice (id_pf_log,id_user,id_pf_revenue,tgl_invoice,no_invoice,id_real_user,id_user_order,ct_invoice,dpp,ppn,dp,npwp) VALUE ('$id_pf_log','$id_user','$id_pf_revenue[$x]','$tgl_invoice','$no_invoice','$id_real_user','$id_user_order','$ct_invoice','$dpp','$ppn','$dp','$npwp')";
