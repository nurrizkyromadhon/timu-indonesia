<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		for($x=0; $x < count($id_pf_revenue); $x++) {	
			...
			$query="INSERT INTO pf_invoice (id_pf,id_user,id_pf_revenue,tgl_invoice,no_invoice,id_real_user,id_user_order,ct_invoice,dpp,ppn,dp) VALUE ('$id_pf','$id_user','$id_pf_revenue[$x]','$tgl_invoice','$no_invoice','$id_real_user','$id_user_order','$ct_invoice','$dpp','$ppn','$dp')";
		...
		?>
			...
			?>
					...
					<div class="col-xs-12">
						...
						<!--<i class="fa fa-globe"></i>--> <h3>INVOICE</h3>
						</h2>
					</div>
					<!-- /.col -->
					</div>
					<?php
							$query2=mysql_query("select * from pf where id_pf=$id_pf");
					...
					<div class="row invoice-info">
						...
						<div class="col-sm-4 invoice-col">
							...
							<?php } 		
								if($hasil['id_real_user']=='0'){
									$query1=mysql_query("select * from pf where id_pf=$id_pf");
									$hasil1=mysql_fetch_array($query1);
						...
						<div class="col-sm-4 invoice-col">
							...
							<b>QUANTITY:</b> <?=$hasil2['qty_pf']?> - <?=$hasil2['type_qty']?><br>
							<b>ROUTE:</b> <?=$hasil2['route_pf']?><br>
							<b>PU/DEL DATE :</b> <?=$hasil2['pudel_date']?><br>
							<b>PU/DEL LOCAION :</b><?=$hasil2['pudel_location']?>
						</div>
						<!-- Pic dan Phone -->
						<div class="col-sm-4 invoice-col">
					...
					<div class="row invoice-info">
						...
						<?php
							if($hasil['id_real_user']!='0'){
									...
									$hasil1=mysql_fetch_array($query1);		
							?>		
							<b>CUSTOMER REFF:</b><?=$hasil2['reff_cust']?> <br>
							<b>CUSTOMER CODE:</b><?=$hasil2['code_cust']?><br>
						...
						<div class="col-sm-4 invoice-col">
							...
							?>
							<!--<b>ETB/ETD :</b> <?=$hasil2['etb']?>/<?=$hasil2['etd']?><br>-->
					...
					<div class="col-md-6 invoice-col">
					   <b>DETAIL KONTAINER :</b> <br>
					        <?php
					            $querynk=mysql_query("select * from detail as d join pf_operasional as po on d.id_pf_operasional=po.id_pf_operasional where desc1 = 'TRANSPORTATION CHARGES' and po.id_pf=$id_pf");
					            while ($hasilnk=mysql_fetch_array($querynk)){
					            echo $hasilnk['no_kontainer'].',';
					            }
					        ?>
