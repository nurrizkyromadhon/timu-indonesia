<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		// Input user
		if ($module=='op_ar_jo' AND $act=='print_op_ar_jo'){
			$id_pf_log=$_GET['id'];
			?>
	...
	break;
		...
		elseif ($module=='op_ar_jo' AND $act=='excel'){
			$id_pf_log=$_GET['id'];
			$date=date('ymd');
		
			header("Content-type: application/vnd-ms-excel");
			header("Content-Disposition: attachment; filename=OpAR($date).xls");
			header("Expires: 0");
		...
		elseif ($module=='op_cash_jo' AND $act=='balik_op_ar'){
			
			$act2=$_POST['act'];
			$id_op_ar=$_POST['id_pf_log'];
		...
		// Update jurnal_keu
		elseif ($module=='op_ar_jo' AND $act=='print_op_ar_jo_all'){
			$tgl_aw=$_GET['tgl_aw'];
			$tgl_ak=$_GET['tgl_ak'];
		?>
			<section class="content-header">
		...
		// Update sor
		elseif ($module=='op_ar_jo' AND $act=='excel_all'){
		$tgl_aw=$_GET['tgl_aw'];
		$tgl_ak=$_GET['tgl_ak'];
		$date=date('ymd');
		
		header("Content-type: application/vnd-ms-excel");
		header("Content-Disposition: attachment; filename=OPARSubAll($date).xls");
		header("Expires: 0");
		header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
		header("Cache-Control: private",false);
		header("Cache-Control: max-age=0");
	?>
		<div>
			<p>LAPORAN OP AR TIMU</p>
            <b class="text-danger">TABEL OP AR </b>
        </div>    
		<table id="myTable" border='1'">
			<thead class="bg-blue">
				<tr>
