...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
 ...
 <center>Untuk mengakses modul, Anda harus login <br>";
	...
	$aksi = "modul/mod_laporan/aksi_hutang_pajak.php";
	switch ($_GET[act]) {
			// Tampil User
		default:
...
?>
			...
			<section class="content">
				...
				<div class="box box-default">
					...
					<div class="box-header with-border row">
						<div class="col-md-6">
							<h3 class="box-title"><b class="text-blue">Tabel Laporan Hutang Pajak (PPN,PPH)</b> dari tgl <strong><?= $tgl_aw_str ?></strong> s/d <strong><?= $tgl_ak_str ?></strong> </h3>
						</div>
						<div class="col-md-6">
							<form name="submit" action="?module=hutang_pajak" method="POST">
							...
							<div class="tabel-responsive">
								<div class="col-md-12">
									<div>
										...
										<b id='jmlOpcash'></b>
										<a class="btn bg-gray btn-sm" href="<?= $aksi ?>?module=hutang_pajak&act=print_hutang_pajak_all&tgl_aw=<?= $tgl_aw ?>&tgl_ak=<?= $tgl_ak ?>" target="_blank"><span class="fa fa-print"></a>
										<a class="btn bg-primary btn-sm" href="<?= $aksi ?>?module=hutang_pajak&act=excel_all&tgl_aw=<?= $tgl_aw ?>&tgl_ak=<?= $tgl_ak ?>"><span class="fa fa-save"></a>
									</div>
									<table id="myTable" class="table table-striped table-bordered">
										...
										<tbody>
											...
											while ($hsl_oc = mysql_fetch_array($qry_oc)) {
												...
												$saldo_ak2 = 0;
												if ($kategori == 'HUT') {
													$qryjml_oc = mysql_query("select * from pf_real_cost where tgl_pf_real_cost between '$tgl_aw' and '$tgl_ak' and category2 = '$kategori' and (kegiatan LIKE 'HUTANG PPH%' or kegiatan LIKE 'HUTANG PPN%')");
											...
											?>
												<tr>
													...
													<td>
														<a class="btn bg-black btn-sm" href="?module=hutang_pajak&act=kategori&kategori=<?= $kategori ?>&tgl_aw=<?= $tgl_aw ?>&tgl_ak=<?= $tgl_ak ?>" target="_blank"><span class="fa fa-eye"></a>
														<a class="btn bg-gray btn-sm" href="<?= $aksi ?>?module=hutang_pajak&act=print_hutang_pajak&kategori=<?= $kategori ?>&tgl_aw=<?= $tgl_aw ?>&tgl_ak=<?= $tgl_ak ?>" target="_blank"><span class="fa fa-print"></a>
														<a class="btn bg-primary btn-sm" href="<?= $aksi ?>?module=hutang_pajak&act=excel&kategori=<?= $kategori ?>&tgl_aw=<?= $tgl_aw ?>&tgl_ak=<?= $tgl_ak ?>"><span class="fa fa-save"></a>
													</td>
												</tr>
											<?php
												$no++;
		...
		case "kategori":
			...
			?>
				...
				<section class="content-header">
					<h1>Hutang Pajak (PPN,PPH)</h1>
					<ol class="breadcrumb">
						<li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
						<li><a href="#">Laporan</a></li>
						<li class="active">Hutang Pajak (PPN,PPH)</li>
					</ol>
				</section>
										...
										<div class="col-md-12">
											<table id="myTable" class="table table-striped table-bordered">
												...
												<tbody>
													...
													$no = 1;
													if ($kategori == 'HUT') {
														$qry = mysql_query("select * from pf_real_cost as prc
                                                		join pf_log as pl on prc.id_pf_log=pl.id_pf_log
                                                		where pl.tgl_pf between '$tgl_aw' and '$tgl_ak' and (kegiatan LIKE 'HUTANG PPH%' or kegiatan LIKE 'HUTANG PPN%') and category2 = '$kategori' order by prc.id_pf_log");
													} else {
														$qry = mysql_query("select * from pf_real_cost as prc
                                                		join pf_log as pl on prc.id_pf_log=pl.id_pf_log
                                                		where pl.tgl_pf between '$tgl_aw' and '$tgl_ak' and (kegiatan LIKE 'BAYAR PPH%' or kegiatan LIKE 'BAYAR PPN%') and category2 = '$kategori' order by prc.id_pf_log");
													}
