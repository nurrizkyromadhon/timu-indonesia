<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
		...
		// Input user
		if ($module=='revenue' AND $act=='print_revenue_jo'){
			?>
			...
			<div class="box-header with-border">
				<h3 class="box-title text-blue text-bold">REVENUE JO</h3>
			</div>
			<div class="box-body">
			...
			<div class="box-header with-border">
				<h3 class="box-title text-blue text-bold">COST JO</h3>
			</div>
			<div class="box-body">
						<table id="table" border="1">
							<thead class="bg-blue">
	...
	break;
		...
		elseif ($module=='revenue' AND $act=='excel'){
			$id_pf_log=$_GET['id'];
			$date=date('ymd');
		
			header("Content-type: application/vnd-ms-excel");
			header("Content-Disposition: attachment; filename=RevenueJO($date).xls");
			header("Expires: 0");
						...
						<table id="table" border="1">
							...
							<tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?=$hsl_oc['desc_revenue']?></td>
                                                    <td><?=$hsl_oc['qty_revenue']?></td>
                                                    <td><?=$hsl_oc['revenue']?></td>
                                                    <td><?=$hsl_oc['qty_revenue'] * $hsl_oc['revenue']?></td>                                                                                                        
                                                    <?php 
		...
		// Update jurnal_keu
		elseif ($module=='revenue' AND $act=='print_revenue_all'){
			$tgl_aw=$_GET['tgl_aw'];
			$tgl_ak=$_GET['tgl_ak'];
		?>
			<section class="content-header">
		...
		// Update sor
		elseif ($module=='revenue' AND $act=='excel_all'){
		$tgl_aw=$_GET['tgl_aw'];
		$tgl_ak=$_GET['tgl_ak'];
		$date=date('ymd');
		
		header("Content-type: application/vnd-ms-excel");
		header("Content-Disposition: attachment; filename=RevenueAll($date).xls");
		header("Expires: 0");
		header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
		header("Cache-Control: private",false);
		header("Cache-Control: max-age=0");
	?>
		<div>
			<p>LAPORAN REVENUE TIMU</p>
            <b class="text-danger">TABEL REVENUE </b>
        </div>    
		<table id="myTable" border='1'">
			<thead class="bg-blue">
				<tr>					
