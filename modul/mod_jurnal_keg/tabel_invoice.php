	...
	if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
	    ...
	    ?>
	        <html>
                ...
                <h4 class="box-title"><b class="text-blue">Tabel Invoice</b> dari tgl <strong><?=$tgl_aw_str?></strong> s/d <strong><?=$tgl_ak_str?></strong> </h4>
                <table id="myTable" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                            $no=1;
                            $qry=mysql_query("SELECT * from pf_invoice as inv 
                            join pf_log on inv.id_pf_log=pf_log.id_pf_log
                            where tgl_invoice between '$tgl_aw' and '$tgl_ak'
                            group by no_invoice order by no_invoice desc");                            
						...
						?>
                                    <tr>
                                        ...
                                        <td align="left"><?=$no_invoice?></td> 
                                        <td><?=$no_jo?></td>
                                        <td align="left"><a class="btn btn-primary btn-sm" href="javascript:insertopr('<?=$hasil['id_pf_invoice']?>').innerHTML" ><span class="fa fa-plus"></a></td>
                ...
                <script>
                    function insertopr(no_invoice) {
                        window.close();
                        window.opener.document.getElementById('id_pf_invoice<?=$_GET['idrow']?>').value = no_invoice;
                    }
